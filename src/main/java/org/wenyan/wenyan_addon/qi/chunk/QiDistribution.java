...
/**
 * 区块灵气结构占比（区域趋同性）：主属性 80%，相生（生/被生）各 7.5%，
 * 其余（相克）各 2.5%。同一群系的所有区块共用同一结构（主属性由群系决定）。
 * 结构占比写入 {@link ChunkQiData#proportions()} 持久化，决定各属性可恢复的比例。
 */
public record QiDistribution(ElementType dominant, ElementType generator,
                             ...
                             double otherRatio) {
    public static final double DOMINANT_RATIO = 0.8;
    public static final double GENERATOR_RATIO = 0.075;
    public static final double GENERATED_RATIO = 0.075;
